...
set EXTENSION_ID=binmboepfajhncgciahmifnlbifphpif
...
echo ID расширения: %EXTENSION_ID%
echo.

if not exist "%EXTENSION_FOLDER%" (
    echo [ОШИБКА] Папка с расширением не найдена!
    echo.
    echo Проверьте путь: %EXTENSION_FOLDER%
    pause
    exit /b 1
)
