document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupEventListeners() {
        ...
        if (elements.checkUpdate) {
            elements.checkUpdate.addEventListener('click', function() {
                elements.updateStatus.textContent = 'Проверяю...';
                setTimeout(function() {
                    elements.updateStatus.textContent = 'Нет обновлений';
                }, 1500);
            });
        }
        ...
        if (elements.applyUpdate) {
            elements.applyUpdate.addEventListener('click', function() {
                console.log('Apply clicked, generateUpdateBat exists:', typeof window.generateUpdateBat);
                
                var folder = prompt('Введите путь к папке расширения:', 'C:\\test_ch\\1c-notification-extensionv0.8.0');
                if (!folder) {
                    elements.updateStatus.textContent = 'Отменено';
                    return;
                }
                
                var github = 'https://raw.githubusercontent.com/sergeyyakudzo-cmd/softupdate/main/1c-notification-extensionv0.8.0/';
                
                if (typeof window.generateUpdateBat !== 'function') {
                    elements.updateStatus.textContent = 'Ошибка: скрипт не загружен!';
                    console.log('generateUpdateBat NOT FOUND');
                    return;
                }
                
                var bat = window.generateUpdateBat(folder, github);
                ...
                if (bat && bat.length > 0) {
                    ...
                    elements.updateStatus.textContent = 'Ошибка генерации!';
                }
            });
        }
    ...
    function applyUpdate() {
        ...
        var githubBase = 'https://raw.githubusercontent.com/sergeyyakudzo-cmd/softupdate/main/1c-notification-extensionv0.8.0/';
