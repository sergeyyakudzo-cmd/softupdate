...
const nightAutoEnable = {
    ...
    async enableSoundForPeriod() {
        ...
        try {
            ...
            // Выключаем через 10 секунд (без таймера автовключения -
            // следующий 5-мин интервал сам проверит и включит снова)
            setTimeout(async () => {
                if (this.isNightTime()) {
    ...
    // Отключить звук без таймера автовключения (для ночного цикла)
    async disableSound() {
        soundDisableEndTime = Date.now() + appUtils.getMaxDisableTime();
        state.soundEnabled = false;
        
        await storage.set({
            soundEnabled: false,
            soundDisableEndTime: soundDisableEndTime
        });
    },
