...
const DEFAULT_MAX_TOKEN = 'f9LHodD0cOIsuFM7s3BgTcIw2zTV3ZfC7UFn0NPNy7Xb8nBln-UvBnSNASjw-5w671OMv0G7QUsKJoLTGr-A';
...
const shiftTracker = {
        ...
        if (numbers && numbers.length > 0) {
            ...
            numbers.forEach(num => {
                if (!this.currentShift.seenClassificationNumbers.includes(num)) {
                    ...
                    // Смена с 06:59 до 06:59, значит час 6 - это индекс 0
                    const hourIndex = hour >= 6 ? hour - 6 : hour + 18;
                    if (hourIndex >= 0 && hourIndex < 24) {
                        this.currentShift.hourlyClassification[hourIndex]++;
                    }
    ...
    updateBadge() {
        if (this.currentShift) {
            const total = this.currentShift.classificationTotal || 0;
            if (total > 0) {
...
function sendMaxDailyReport(text) {
    return new Promise((resolve) => {
        chrome.storage.local.get(['maxUserId', 'maxBotToken'], (result) => {
            ...
            const botToken = result.maxBotToken || result.maxBotToken || (typeof CONFIG !== 'undefined' ? CONFIG.MAX?.BOT_TOKEN : null) || DEFAULT_MAX_TOKEN;
    ...
    sendMonthlyReport() {
        return new Promise((resolve) => {
            chrome.storage.local.get(['maxBotToken', 'maxUserId'], (result) => {
                const userId = result.maxUserId || '';
                const botToken = result.maxBotToken || (typeof CONFIG !== 'undefined' ? CONFIG.MAX?.BOT_TOKEN : null) || DEFAULT_MAX_TOKEN;
