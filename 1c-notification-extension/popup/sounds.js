...
function loadSettings() {
    chrome.storage.local.get([
        ...
        'classSoundEnabled', 'groupSoundEnabled'
    ], (result) => {
        try {
    ...
    sendMessageToContentScript({ type: 'GET_SOUND_INFO' })
        .then(response => {
            ...
            if (r && !r.error) {
                ...
                updateSoundToggle();
                if (r.classSoundEnabled !== undefined) state.classSoundEnabled = !!r.classSoundEnabled;
                if (r.groupSoundEnabled !== undefined) state.groupSoundEnabled = !!r.groupSoundEnabled;
                updateMetricSoundButtons();
...
function updateMetricSoundButtons() {
    const classBtn = elements.classificationSoundToggle;
    const groupBtn = elements.groupSoundToggle;
    if (classBtn) {
        const icon = classBtn.querySelector('i');
        const muted = !state.isSoundEnabled || !state.classSoundEnabled;
        classBtn.classList.toggle('muted', muted);
        if (icon) icon.className = muted ? 'fas fa-volume-mute' : 'fas fa-volume-up';
    }
    if (groupBtn) {
        const icon = groupBtn.querySelector('i');
        const muted = !state.isSoundEnabled || !state.groupSoundEnabled;
...
function toggleChannelSound(channel) {
    ...
    const type = isClass ? 'SET_CLASS_SOUND_ENABLED' : 'SET_GROUP_SOUND_ENABLED';
    sendMessageToContentScript({ type, enabled })
        .then(() => {
            if (isClass) state.classSoundEnabled = enabled;
            else state.groupSoundEnabled = enabled;
            chrome.storage.local.set(isClass ? { classSoundEnabled: enabled } : { groupSoundEnabled: enabled });
            updateMetricSoundButtons();
            showToast((isClass ? 'Звук классификации' : 'Звук группы') + (enabled ? ' включён' : ' выключен'),
...
export {
    ...
    updateSoundVolume, updateGroupVolume, updateMetricSoundButtons, toggleChannelSound,
    getDefaultSoundOptions, getDefaultGroupSoundOptions,
};
