...
function loadSettings() {
    chrome.storage.local.get([
        ...
        try {
            ...
            state.classSoundEnabled = result.classSoundEnabled !== false;
            state.groupSoundEnabled = result.groupSoundEnabled !== false;
    ...
    sendMessageToContentScript({ type: 'GET_SOUND_INFO' })
        .then(response => {
            ...
            if (r && !r.error) {
                ...
                state.classSoundEnabled = r.classSoundEnabled !== false;
                state.groupSoundEnabled = r.groupSoundEnabled !== false;
...
function updateMetricSoundButtons() {
    if (elements.classificationSoundToggle) {
        const icon = elements.classificationSoundToggle.querySelector('i');
        const muted = !state.classSoundEnabled;
        if (icon) icon.className = muted ? 'fas fa-volume-mute' : 'fas fa-volume-up';
        elements.classificationSoundToggle.classList.toggle('muted', muted);
        elements.classificationSoundToggle.title = muted ? 'Звук классификации выключен' : 'Звук классификации включён';
    }
    if (elements.groupSoundToggle) {
        const icon = elements.groupSoundToggle.querySelector('i');
        const muted = !state.groupSoundEnabled;
        if (icon) icon.className = muted ? 'fas fa-volume-mute' : 'fas fa-volume-up';
        elements.groupSoundToggle.classList.toggle('muted', muted);
        elements.groupSoundToggle.title = muted ? 'Звук группы выключен' : 'Звук группы включён';
    }
}

function toggleChannelSound(channel) {
    if (!state.currentTab) {
        showToast('Сначала откройте страницу 1С', 'warning');
        return;
    }
    const isClass = channel === 'class';
    const enabled = !(isClass ? state.classSoundEnabled : state.groupSoundEnabled);
    const messageType = isClass ? 'SET_CLASS_SOUND_ENABLED' : 'SET_GROUP_SOUND_ENABLED';
    sendMessageToContentScript({ type: messageType, enabled: enabled })
        .then(response => {
            if (response && response.success) {
                if (isClass) state.classSoundEnabled = enabled;
                else state.groupSoundEnabled = enabled;
                updateMetricSoundButtons();
                showToast(
                    `${isClass ? 'Звук классификации' : 'Звук группы'} ${enabled ? 'включён' : 'выключен'}`,
...
function toggleClassSoundHandler() { toggleChannelSound('class'); }
function toggleGroupSoundHandler() { toggleChannelSound('group'); }
...
export {
    ...
    updateMetricSoundButtons, toggleClassSoundHandler, toggleGroupSoundHandler,
};
