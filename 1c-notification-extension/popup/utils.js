...
function renderMonthlyChart() {
    ...
    chrome.storage.local.get(['monthlyStatsData'], (result) => {
        const dataStore = result.monthlyStatsData;
        const monthKey = getShiftDateKey().slice(0, 7);
        let month = (dataStore && dataStore.months) ? dataStore.months[monthKey] : null;
        let days = month && month.dailyStats ? Object.keys(month.dailyStats).sort() : [];

        const classH = state.hourlyClassification || [];
        const groupH = state.hourlyGroup || [];
        const liveTotal = classH.reduce((a, b) => a + (b || 0), 0) + groupH.reduce((a, b) => a + (b || 0), 0);
        const todayKey = getShiftDateKey();
        if (liveTotal > 0) {
            if (!month) { month = { dailyStats: {} }; }
            month.dailyStats[todayKey] = { total: liveTotal };
        }
        if (month && month.dailyStats) days = Object.keys(month.dailyStats).sort();
        ...
        const data = days.map(d => (month.dailyStats[d].total || 0));
        const dk = data.join(',');
        if (container.dataset.lastData === dk && container.querySelector('.chart-bars') && container.dataset.forceRender !== 'true') return;
        delete container.dataset.forceRender;
        container.dataset.lastData = dk;
        ...
        const labelEvery = days.length > 14 ? 5 : (days.length > 7 ? 3 : 1);
        const chartPx = 140;
        let html = '<div class="chart-wrap"><div class="chart-ylabels">';
        yLabels.forEach(v => { html += `<span class="chart-ylabel">${v}</span>`; });
        html += '</div><div class="chart-bars">';
        ...
        data.forEach((val, i) => {
            ...
            const showLabel = i === days.length - 1 || i % labelEvery === 0;
            html += `<div class="chart-col${isLast ? ' chart-bar-last' : ''}" style="--i:${i}">
                <div class="chart-bar" style="height:${barH}px">${val > 0 ? `<span class="chart-bar-label">${val}</span>` : ''}</div>
                <span class="chart-hour${showLabel ? '' : ' chart-hour-placeholder'}">${xLabels[i]}</span></div>`;
        });
        html += '</div></div>';
        container.innerHTML = html;
    });
...
function renderChart() {
    const container = document.getElementById('hourlyChartContainer');
    if (!container) return;
    const isMonth = state.chartPeriod === 'month';
    container.classList.toggle('chart-mode-month', isMonth);
    if (isMonth) renderMonthlyChart();
