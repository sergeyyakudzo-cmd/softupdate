...
function renderMonthlyChart() {
    const container = document.getElementById('hourlyChartContainer');
    if (!container) return;
    container.classList.add('chart-mode-month');
    chrome.storage.local.get(['monthlyStatsData'], (result) => {
        const months = (result.monthlyStatsData && result.monthlyStatsData.months) || {};
        const monthKey = getShiftDateKey().slice(0, 7);
        const month = months[monthKey] || null;
        const storedDays = (month && month.dailyStats) || {};
        const todayKey = getShiftDateKey();
        const liveClass = (state.hourlyClassification || []).reduce((a, v) => a + (v || 0), 0);
        const liveGroup = (state.hourlyGroup || []).reduce((a, v) => a + (v || 0), 0);
        const liveTotal = liveClass + liveGroup;
        const daily = { ...storedDays };
        if (liveTotal > 0) {
            daily[todayKey] = { total: liveTotal };
        }
        const days = Object.keys(daily).filter(d => d.startsWith(monthKey)).sort();
        if (!days.length) {
            container.innerHTML = '<div class="chart-empty">Нет данных за месяц</div>';
            container.dataset.lastData = 'empty';
            return;
        }
        const data = days.map(d => ({ date: d, total: daily[d].total || 0 }));
        const dataMax = Math.max(...data.map(d => d.total), 1);
        const labelEvery = data.length > 14 ? 5 : (data.length > 7 ? 3 : 1);
        const dk = data.map(d => d.date + ':' + d.total).join(',');
        if (container.dataset.lastData === dk && container.querySelector('.chart-bars') && container.dataset.forceRender !== 'true') return;
        delete container.dataset.forceRender;
        container.dataset.lastData = dk;
        ...
        data.forEach((d, i) => {
            ...
            const barH = Math.max(2, (d.total / dataMax) * 140);
            const label = String(parseInt(d.date.slice(8), 10));
            const showLabel = isToday || (labelEvery === 1) || (i % labelEvery === 0) || i === data.length - 1;
            const mmdd = d.date.slice(5);
            const cls = 'chart-col' + (isToday ? ' chart-bar-last' : '');
            html += `<div class="${cls}" style="--i:${i}">
                <div class="chart-bar" style="height:${barH}px">${d.total > 0 ? `<span class="chart-bar-label">${d.total}</span>` : ''}</div>
                <span class="chart-hour${showLabel ? '' : ' chart-hour-placeholder'}" title="${mmdd}">${showLabel ? label : '&nbsp;'}</span></div>`;
        });
        html += '</div></div>';
        container.innerHTML = html;
    });
...
function renderChart() {
    const container = document.getElementById('hourlyChartContainer');
    if (container) container.classList.toggle('chart-mode-month', state.chartPeriod === 'month');
    if (state.chartPeriod === 'month') renderMonthlyChart();
    else renderHourlyChart();
}

function updateChartToggle() {
    document.querySelectorAll('.chart-toggle-btn').forEach(btn => {
        const active = btn.dataset.period === state.chartPeriod;
...
export {
    ...
    updateCheckTimer, renderHourlyChart, renderMonthlyChart, renderChart, updateChartToggle,
    updateShiftTimer, getShiftDateKey,
