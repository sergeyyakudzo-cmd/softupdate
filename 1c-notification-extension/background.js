...
// ============ СМЕННЫЙ УЧЕТ ЗАЯВОК (07:00 - 07:00) ============
const shiftTracker = {
    currentShift: null, // { date: 'YYYY-MM-DD', classification: 0, group: 0 }
    _loaded: false,
    _pendingUpdates: [],
    
    // Получить текущую дату смены (с 07:00 до 07:00)
    getShiftDate() {
        const now = new Date();
        const hours = now.getHours();
        
        // Если сейчас меньше 07:00, смена еще со вчерашнего дня
        if (hours < 7) {
    ...
    initShift() {
        ...
        if (!this.currentShift || this.currentShift.date !== shiftDate) {
            ...
            this.currentShift = {
                ...
                seenInGroup: [],
                // Часовые срезы (07:00-07:00 следующего дня = 24 часа)
                ...
                currentGroupCount: 0,
                // Пик классификации (максимум одновременных заявок)
                peakClassification: 0,
                peakClassificationTime: null
            };
    ...
    processApplications(classNumbers = [], groupNumbers = []) {
        ...
        // Обновляем пик классификации (максимум одновременных заявок)
        if (this.currentShift.currentClassificationCount > this.currentShift.peakClassification) {
            this.currentShift.peakClassification = this.currentShift.currentClassificationCount;
            this.currentShift.peakClassificationTime = new Date().toISOString();
        }
        ...
        chrome.storage.local.get(['shiftData'], (result) => {
            if (result.shiftData) {
                ...
                if (result.shiftData.date === shiftDate) {
                    ...
                    if (this.currentShift.peakClassification === undefined) {
                        this.currentShift.peakClassification = 0;
                        this.currentShift.peakClassificationTime = null;
                    }
                } else {
    ...
    generateHourlyChart(shift) {
        ...
        // Часы с 07:00 до 19:00 - классификация, 19:00-07:00 - группы
...
function scheduleDailyReport() {
    ...
    // 07:00 - конец смены, отправляем отчет и начинаем новую смену
    targetTime.setHours(7, 0, 0, 0);
    
    // Если уже прошло 07:00, планируем на завтра
    if (targetTime <= now) {
...
function cdpDetach() {
    ...
    return new Promise((resolve) => {
        chrome.debugger.detach({ tabId: cdpAttachedTabId }, () => {
...
function cdpSendCommand(method, params) {
    return new Promise((resolve, reject) => {
        chrome.debugger.sendCommand({ tabId: cdpAttachedTabId }, method, params, (result) => {
...
function cdpKey(key) {
    return cdpSendCommand('Input.dispatchKeyEvent', {
        type: 'keyDown', key
    }).then(() => cdpSendCommand('Input.dispatchKeyEvent', {
        type: 'keyUp', key
    }));
}
