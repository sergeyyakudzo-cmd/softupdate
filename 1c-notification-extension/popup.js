document.addEventListener('DOMContentLoaded', function() {
    ...
    const elements = {
        ...
        // Telegram настройки
        telegramSettingsModal: document.getElementById('telegramSettingsModal'),
        closeTelegramSettings: document.getElementById('closeTelegramSettings'),
        saveTelegramSettings: document.getElementById('saveTelegramSettings'),
        clearTelegramSettings: document.getElementById('clearTelegramSettings'),
        sendTestTelegram: document.getElementById('sendTestTelegram'),
        telegramConnectionType: document.getElementById('telegramConnectionType'),
        telegramProxyType: document.getElementById('telegramProxyType'),
        telegramProxyHost: document.getElementById('telegramProxyHost'),
        telegramProxyPort: document.getElementById('telegramProxyPort'),
        telegramProxyUsername: document.getElementById('telegramProxyUsername'),
        telegramProxyPassword: document.getElementById('telegramProxyPassword'),
        telegramCustomApiUrl: document.getElementById('telegramCustomApiUrl')
    };
    ...
    function initTabs() {
        ...
        tabButtons.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                // Обновить sparklines при переключении на stats
                if (tabId === 'stats') {
                    updateStatsSparklines();
                }
            });
        });
    }
    ...
    function updateStatsSparklines() {
        const classContainer = document.getElementById('classificationSparklineStats');
        const groupContainer = document.getElementById('groupSparklineStats');

        if (!classContainer || !groupContainer) return;

        renderSparkline('classificationSparkSvgStats', state.classificationHistory, '#2563eb');
        renderSparkline('groupSparkSvgStats', state.groupHistory, '#6366f1');
    }
    ...
    function setupEventListeners() {
        ...
        if (elements.checkUpdate) {
            elements.checkUpdate.addEventListener('click', function() {
                ...
                const currentVersion = document.querySelector('.app-version-badge')?.textContent || 'v1.0.0';
        ...
        if (elements.applyUpdate) {
            elements.applyUpdate.addEventListener('click', function() {
                ...
                lines.push('echo 1.0.0 > "%EXT%\\version.txt"');
        ...
        if (elements.closeTelegramSettings) elements.closeTelegramSettings.addEventListener('click', hideTelegramSettings);
        if (elements.telegramSettingsModal) {
            elements.telegramSettingsModal.addEventListener('click', (event) => {
                if (event.target === elements.telegramSettingsModal) {
