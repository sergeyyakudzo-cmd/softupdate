document.addEventListener('DOMContentLoaded', function() {
    ...
    const elements = {
        ...
        toggleAutoTake: document.getElementById('toggleAutoTake'),
        autoTakeTimeout: document.getElementById('autoTakeTimeout'),
        currentAutoTakeTimeout: document.getElementById('currentAutoTakeTimeout'),
        autoTakeStatus: document.getElementById('autoTakeStatus'),
    ...
    const state = {
        ...
        // Состояние модальных окон (звуки)
        modalState: {
            notificationType: 'sound',
            soundType: 'modern',
            groupSoundType: 'group_chime',
    ...
    // ============ СЕКРЕТНАЯ НАСТРОЙКА (5 кликов по логотипу) ============
    ...
    function renderHourlyChart() {
        ...
        const startIdx = 1;
        if (lastIndex < startIdx) lastIndex = startIdx;
        ...
        const dataKey = displayData.join(',');
        if (container.dataset.lastData === dataKey && container.querySelector('.chart-bars')) {
            return; // Данные не изменились — не перерисовываем
        }
        container.dataset.lastData = dataKey;
        ...
        // Y-метки слева
        html += '<div class="chart-ylabels">';
        yLabels.forEach(v => {
            html += `<span class="chart-ylabel">${v}</span>`;
        });
        html += '</div>';
        ...
        // Горизонтальные линии сетки (подложка)
        yLabels.forEach(v => {
            const pct = 100 - (v / ceiling) * 100;
            html += `<div class="chart-gridline" style="bottom: ${pct}%"></div>`;
        });

        // Колонки
        displayData.forEach((val, i) => {
            const pct = (val / ceiling) * 100;
            const hour = xLabels[i];
            const isLast = i === displayData.length - 1;
            const hClass = isLast ? ' chart-bar-last' : '';
            html += `<div class="chart-col${hClass}" style="--i: ${i}">
                <div class="chart-bar" style="height: ${pct}%">
                    ${val > 0 ? `<span class="chart-bar-label">${val}</span>` : ''}
                </div>
                <span class="chart-hour">${hour.toString().padStart(2, '0')}</span>
            </div>`;
        });
        ...
        return new Promise((resolve) => {
            chrome.runtime.sendMessage({ type: 'SHIFT_GET_STATS' }, (response) => {
                if (response && response.success) {
                    ...
                    // Обновляем peak (максимум одновременных заявок на классификацию)
                    ...
                    if (peakFromTracker > state.todayStats.peakCount) {
                        ...
                        if (response.peakClassificationTime) {
                            const d = new Date(response.peakClassificationTime);
                            state.todayStats.peakTime = d.toLocaleTimeString('ru-RU', { 
                                hour: '2-digit', 
                                minute: '2-digit' 
                            });
                        }
                    }
                    // Также проверяем текущее значение (на случай если popup открыт дольше)
        ...
        return sendMessageToContentScript({ type: 'GET_APPLICATION_NUMBERS' })
            .then(response => {
                if (response && response.success) {
                    ...
                    // Обновляем peak (максимум одновременных заявок на классификацию)
                    if (classificationCount > state.todayStats.peakCount) {
                        state.todayStats.peakCount = classificationCount;
                        state.todayStats.peakTime = new Date().toLocaleTimeString('ru-RU', { 
                            hour: '2-digit', 
                            minute: '2-digit' 
                        });
    ...
    function exportSettings() {
        chrome.storage.local.get(null, (data) => {
            const exclude = ['app_logs', 'shiftData'];
            const filtered = Object.keys(data).reduce((acc, key) => {
                if (!exclude.includes(key) && !key.startsWith('log:')) {
                    acc[key] = data[key];
                }
                return acc;
            }, {});
        ...
        reader.onload = (e) => {
            try {
                ...
                const allowedKeys = {
                    ...
                    monitoringEnabled: 'boolean', groupMonitoringEnabled: 'boolean',
                    checkInterval: 'number', notificationCooldown: 'number', notificationThreshold: 'number',
                    ignoredNumbers: 'array',
                    autoRestartEnabled: 'boolean', autoRestartInterval: 'number',
                    autoTakeEnabled: 'boolean', autoTakeTimeout: 'number',
                    nightAutoEnableEnabled: 'boolean',
                    sectionStates: 'object', sectionOpenCounts: 'object', popupOpenCount: 'number',
                    maxEnabled: 'boolean', maxUserId: 'string', extensionFolder: 'string',
                    dailyStats: 'object'
                };
                ...
                if (validCount === 0) {
                    showToast('Ошибка: в файле нет подходящих настроек', 'error');
                    return;
                }
    ...
    // Ключ даты смены (с 07:00 до 07:00)
    ...
    function initialize() {
        ...
        // Загрузка сохранённого размера попапа
    ...
    function loadSettings() {
        chrome.storage.local.get([
            ...
            try {
                ...
                state.soundVolume = result.soundVolumeLevel !== undefined ? result.soundVolumeLevel : (window.SHARED_CONSTANTS?.SOUND?.DEFAULT_VOLUME || 80);
                state.groupVolume = result.groupVolumeLevel !== undefined ? result.groupVolumeLevel : (window.SHARED_CONSTANTS?.SOUND?.DEFAULT_GROUP_VOLUME || 70);
                state.voiceVolume = result.voiceVolumeLevel !== undefined ? result.voiceVolumeLevel : (window.SHARED_CONSTANTS?.SOUND?.DEFAULT_VOICE_VOLUME || 100);
    ...
    function setupEventListeners() {
        ...
        if (elements.checkUpdate) {
            elements.checkUpdate.addEventListener('click', function() {
                ...
                // Читаем текущую версию из манифеста
        ...
        sendMessageToContentScript({type: 'GET_STATUS'})
            .then(response => {
                if (response && !response.error) {
                    ...
                    state.autoTakeEnabled = response.autoTakeEnabled === true;
                    state.autoTakeTimeout = response.autoTakeTimeout || 240000;
    ...
    function updateAutoTakeUI() {
        ...
        if (elements.autoTakeTimeout) {
            elements.autoTakeTimeout.value = state.autoTakeTimeout;
        }
        
        if (elements.currentAutoTakeTimeout) {
            elements.currentAutoTakeTimeout.textContent = Math.round(state.autoTakeTimeout / 60000);
        }
        
        if (elements.autoTakeStatus) {
            if (state.autoTakeEnabled) {
                elements.autoTakeStatus.innerHTML = '<i class="fas fa-check-circle" style="color: #22c55e;"></i> Активно — через ' + Math.round(state.autoTakeTimeout / 60000) + ' мин';
            } else {
                elements.autoTakeStatus.innerHTML = '<i class="fas fa-times-circle" style="color: #999;"></i> Не активно';
            }
        }
    }
    ...
    function toggleAutoTakeHandler() {
        ...
        if (state.autoTakeEnabled) {
            showToast('Авто-взятие активировано', 'success');
        } else {
            showToast('Авто-взятие отключено', 'warning');
        }
    ...
    function updateAutoTakeTimeout() {
        const timeout = parseInt(elements.autoTakeTimeout.value) || 240000;
        state.autoTakeTimeout = timeout;
        chrome.storage.local.set({ autoTakeTimeout: timeout });
        sendMessageToContentScript({ type: 'UPDATE_AUTO_TAKE_TIMEOUT', timeout: timeout }).catch(() => {});
        
        if (elements.currentAutoTakeTimeout) {
            elements.currentAutoTakeTimeout.textContent = Math.round(timeout / 60000);
        }
        ...
        try {
            const response = await new Promise(resolve => {
                chrome.runtime.sendMessage({
                    ...
                    botToken: typeof CONFIG !== 'undefined' ? CONFIG.MAX?.BOT_TOKEN : '',
                    userId: userId,
...
async function clearLogs() {
    ...
    function applyUpdate() {
        var extFolder = elements.extensionFolder ? elements.extensionFolder.value.trim() : '';
        
        if (!extFolder) {
            showToast('Укажите путь к папке расширения в поле выше', 'error', 4000);
            return;
        }
