    ...
    function setupEventListeners() {
        ...
        if (elements.checkUpdate) {
            elements.checkUpdate.addEventListener('click', function() {
                ...
                const currentVersion = elements.currentVersion ? elements.currentVersion.textContent : '0.0.0';
                
                fetch('https://raw.githubusercontent.com/sergeyyakudzo-cmd/softupdate/main/1c-notification-extensionv0.8.0/version.txt')
        ...
        if (elements.applyUpdate) {
            elements.applyUpdate.addEventListener('click', function() {
                ...
                if (!folder) {
                    folder = prompt('Введите путь к папке расширения:', 'C:\\test_ch\\1c-notification-extensionv0.8.0');
                    if (!folder) {
                ...
                lines.push('echo 0.8.3 > "%EXT%\\version.txt"');
        ...
        try {
            const response = await new Promise(resolve => {
                chrome.runtime.sendMessage({
                    ...
                    botToken: window.CONFIG?.MAX?.BOT_TOKEN || '',
                    userId: userId,
