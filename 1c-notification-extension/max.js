...
const MAX_CONFIG = {
    ...
    // Токен бота MAX - из config.js
    BOT_TOKEN: window.CONFIG?.MAX?.BOT_TOKEN || ''
};
...
const max = {
    ...
    botToken: '',
    userId: '',
    messageTemplate: window.SHARED_CONSTANTS?.MAX?.DEFAULT_TEMPLATE || '🔔 В {time} обнаружено {count} новых заявок на {type}',
    groupMessageTemplate: window.SHARED_CONSTANTS?.MAX?.DEFAULT_GROUP_TEMPLATE || '👥 В {time} обнаружено {count} задач в группах',
    ...
    async init() {
        try {
            ...
            this.messageTemplate = data.maxMessageTemplate || window.SHARED_CONSTANTS?.MAX?.DEFAULT_TEMPLATE || '🔔 В {time} обнаружено {count} новых заявок на {type}';
            this.groupMessageTemplate = data.maxGroupMessageTemplate || window.SHARED_CONSTANTS?.MAX?.DEFAULT_GROUP_TEMPLATE || '👥 В {time} обнаружено {count} задач в группах';
